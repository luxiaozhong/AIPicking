...
#
# Cron（每5分钟，工作日盘中）:
#   */5 9-11 * * 1-5 cd /opt/AIpicking/backend && bash scripts/sync_intraday_daily.sh >> /var/log/aipicking/update_daily_intraday.log 2>&1
#   */5 13-14 * * 1-5 cd /opt/AIpicking/backend && bash scripts/sync_intraday_daily.sh >> /var/log/aipicking/update_daily_intraday.log 2>&1
...
# ── 2. 成分股日线（按指数过滤） ──
if [ $# -eq 0 ]; then
    INDICES=(980080 900001 900002 399667)
else
    INDICES=("$@")
fi
