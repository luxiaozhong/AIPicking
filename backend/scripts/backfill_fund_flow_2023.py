#!/usr/bin/env python3
"""
2023年个股资金流 backfill 脚本（断点续传）
============================================

逐日调用 sync_stock_fund_flow 的 sync() 函数，补全 2023 年缺失的 daily_stock_fund_flow 数据。
每完成一天写入进度文件，支持 Ctrl+C 中断后重启自动续传。

用法:
    cd backend && source venv/bin/activate
    python scripts/backfill_fund_flow_2023.py              # 自动从进度文件恢复
    python scripts/backfill_fund_flow_2023.py --reset      # 忽略进度，重新开始
    python scripts/backfill_fund_flow_2023.py --dry-run    # 仅打印计划

进度文件: backend/scripts/.backfill_fund_flow_2023_progress.json
日志文件: backend/scripts/backfill_fund_flow_2023.log
"""
...
PROGRESS_FILE = SCRIPT_DIR / ".backfill_fund_flow_2023_progress.json"
LOG_FILE = SCRIPT_DIR / "backfill_fund_flow_2023.log"
...
def get_missing_dates() -> list[str]:
    """Return sorted list of 2023 trading dates missing from daily_stock_fund_flow."""
    conn = get_db_conn()
    try:
        cur = conn.cursor()
...
def run_backfill(dry_run: bool = False, reset: bool = False):
    ...
    if not missing:
        logger.info("✅ 2023年资金流数据已完整，无需 backfill！")
        return
...
# ── CLI ────────────────────────────────────────────────────────────
def main():
    p = argparse.ArgumentParser(description="2023年个股资金流 backfill（断点续传）")
