...
def parse_log(path: str, is_remote: bool = False) -> dict:
    ...
    Supports two formats:
      A: [N/TOTAL] DATE — ✓ 完成 (Xs)  /  [N/TOTAL] DATE — 开始 (HH:MM)
      B: [N/TOTAL] 同步 DATE ... HH:MM:SS  (prod 120-day script)
    """
    if is_remote:
        out = ssh_output(f'tail -300 "{path}" 2>/dev/null')
    else:
        try:
            with open(path) as f:
                file_lines = f.readlines()
            out = "".join(file_lines[-300:])
    ...
    # Try format A: "[N/TOTAL] DATE — ✓ 完成 (Xs)" or "[N/TOTAL] DATE — 开始"
    ...
    if fmt_a:
        ...
        if "完成" in action:
            result["status"] = "✅ 完成" if result["current"] == result["total"] else "🟢 运行中"
        elif "开始" in action:
            result["status"] = "🟢 运行中"
        elif "失败" in action:
            result["status"] = "🔴 失败"
        ...
        # Parse format B (no explicit "完成" marker — sequential lines)
        result["total"] = int(fmt_b[0][1])
        last = fmt_b[-1]
        result["current"] = int(last[0])
        result["current_date"] = last[2]
        result["pct"] = round(result["current"] / result["total"] * 100, 1)
        result["done"] = result["current"] - 1  # current line is in-progress

        # Duration can't be extracted from format B
...
def compose_html(lines: list[dict]) -> str:
    ...
    for l in lines:
        ...
        <tr>
            <td>{l['name']}</td>
            <td style="text-align:center">{l['done'] or '—'}/{l['total'] or '—'}</td>
        ...
        if l['pct'] is not None:
            ...
            <div style="margin-bottom:8px">
                ...
                <div style="background:#eee;border-radius:4px;height:6px">
                    <div style="background:{l.get('color','#3498db')};border-radius:4px;height:6px;width:{min(l['pct'],100)}%"></div>
                </div>
            </div>"""
...
def collect_all() -> list[dict]:
    """Collect progress from all 4 backfill lines."""
    lines = []

    # ① Local 2026
    l1 = parse_log("/tmp/backfill_fund_flow.log")
    l1["name"] = "① 本地 2026线"
    l1["color"] = "#3498db"
    lines.append(l1)

    # ② Local 2025
    l2 = parse_log("/tmp/backfill_fund_flow_2025.log")
    l2["name"] = "② 本地 2025线"
    l2["color"] = "#e67e22"
    lines.append(l2)

    # ③ Prod 120天
    l3 = parse_log("/var/log/aipicking/fund_flow_backfill.log", is_remote=True)
    l3["name"] = "③ Prod 120天"
    l3["color"] = "#27ae60"
    lines.append(l3)

    # ④ Prod 2025
    l4 = parse_log("/tmp/backfill_fund_flow_2025.log", is_remote=True)
    l4["name"] = "④ Prod 2025线"
    l4["color"] = "#9b59b6"
    lines.append(l4)
...
def main():
    ...
    for l in lines:
        pct = f"{l['pct']}%" if l['pct'] is not None else "—"
        dur = fmt_duration(l["last_duration"])
        print(f"  {l['name']}: {l['done']}/{l['total']} ({pct}) | {l['current_date'] or '—'} | {dur} | {l['status']}")
