"""
超跌反弹策略 (Oversold Bounce)
捕捉放量急跌 → 缩量止跌 → 放量反弹的三阶段信号链。

原始逻辑来源：oversold_bounce.py（外部系统）
...
# 大盘择时
MARKET_TIMING = True
MARKET_INDEX = "399006.SZ"  # 创业板指（需带后缀）
...
def run(data):
    """
    AIpicking 策略接口 — 超跌反弹选股。

    Args:
        data: {
            "cutoff_date": "20260525",
            "stocks": [{"ts_code": "300001.SZ", "name": "特锐德", "market": "创业板",
                         "float_shares": 1200000000, "total_shares": 2000000000, ...}],
            "daily": {
                "300001.SZ": [{"trade_date": "20260101", "open": 10.0, "high": 10.5,
    ...
    Returns:
        [{"ts_code": "300001.SZ", "name": "特锐德", "score": 85,
    ...
    for ts_code, rows in daily.items():
        ...
        # ── 股票池过滤：创业板(300/301) + 科创板(688/689) ──
        if not (
            ts_code.startswith("300")
            or ts_code.startswith("301")
            or ts_code.startswith("688")
