#!/usr/bin/env python3
"""
grow_with_money · 每日调仓回测 · 980080

策略：每个交易日从国证成长100成分股中按过去 M 日主力净流入排名取 Top N，
...
def run(top_n, lookback, index_code="980080", stop_loss_pct=0,
    ...
    # missing dates → last known close; is_tradable = real row exists
    is_tradable = {}   # {date: {code: bool}}
    last_px = {}       # {code: (last_date, last_close)}
    for td_ in all_td:
        px_day = pr.get(td_, {})
        is_tradable[td_] = {}
        for c in match_ts:
    ...
    for i, td in enumerate(bt):
        ...
        if i==0:
            ...
            for s in tradable_ranking:
                ...
                if p>0: holdings[s["ts_code"]]=(start_nav*w)/p; cost_basis[s["ts_code"]]=p
        ...
        skipped_buys = len(buy_c) - len(actual_buys)
        if actual_buys and cash>0:
            ca = cash/len(actual_buys)
            for c in actual_buys:
                p = px.get(c,0)
                if p>0:
                    holdings[c]=(ca*(1-BUY_C))/p; cost_basis[c]=p
        ...
        ct = sg*SELL_C + bct
        dr = (nav/last_nav-1)*100 if last_nav>0 else 0

        is_fri = datetime.strptime(td,"%Y-%m-%d").weekday()==4
        if is_fri or i<=3:
            print(f"{td:<12} {nav:>10.2f} {dr:>7.2f}% {len(holdings):>5} {len(keep_c):>5} {sold_today:>5} {len(actual_buys):>5} {to:>6.1f}% {ct:>8.2f}")
