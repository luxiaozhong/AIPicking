"""
超跌反弹策略 批量回测脚本（2022-2025，1日/3日持有期）
======================================================
与 batch_backtest_2022_2025.py 逻辑相同，仅持有期从 7日/15日 改为 1日/3日。

用法: cd backend && source venv/bin/activate && python tmpScriptsBackTest/batch_backtest_2022_2025_1d_3d.py
"""
...
YEAR_START = "20220101"
YEAR_END = "20251231"
DATA_START = "20210701"  # 180天前
DATA_END = "20260215"    # 15天缓冲
...
# ── 持有期：1日 / 3日 ──
TRACK_DAYS = [0, 1, 3]
TRACK_LABELS = [(0, "ret_0d"), (1, "ret_1d"), (3, "ret_3d")]
...
def generate_html(all_results, all_index_data, forward_prices, index_forward,
    ...
    for cutoff_date, recs in all_results.items():
        for r in recs:
            ...
            for days, label in TRACK_LABELS:
                fd = fp.get(days)
    ...
    all_1d = [p["ret_1d"] for p in stock_performances if p["ret_1d"] is not None]
    all_3d = [p["ret_3d"] for p in stock_performances if p["ret_3d"] is not None]
    ...
    year_groups = [
        ...
        ("2024年", [d for d in sorted(all_results.keys()) if d.startswith("2024")]),
        ("2025年", [d for d in sorted(all_results.keys()) if d.startswith("2025")]),
    ]
    ...
    html.append("<title>超跌反弹策略 2022-2025 批量回测报告（1日/3日持有期）</title>")
    html.append(CSS)
    html.append("</head><body>")

    html.append("<h1>📊 超跌反弹策略 2022-2025 批量回测报告（1日/3日持有期）</h1>")
    html.append("<div class='meta'>")
    html.append(f"<p><strong>生成时间：</strong>{now_str}</p>")
    html.append(f"<p><strong>回测范围：</strong>2022-01-01 ~ 2025-12-31（{len(trading_days_list)} 个交易日）</p>")
    ...
    html.append(f"<p><strong>持有期：</strong>1日 / 3日</p>")
    ...
    avg1_val = sum(all_1d) / len(all_1d) if all_1d else 0
    up1 = sum(1 for v in all_1d if v > 0) if all_1d else 0
    html.append(f"<div class='stat-card'><div class='label'>1日平均收益</div><div class='value' style='color:#{'e74c3c' if avg1_val>=0 else '27ae60'}'>{avg1_val:+.2f}%</div><div class='sub'>上涨率 {up1}/{len(all_1d)} ({up1/len(all_1d)*100:.0f}%)</div></div>")
    avg3_val = sum(all_3d) / len(all_3d) if all_3d else 0
    up3 = sum(1 for v in all_3d if v > 0) if all_3d else 0
    html.append(f"<div class='stat-card'><div class='label'>3日平均收益</div><div class='value' style='color:#{'e74c3c' if avg3_val>=0 else '27ae60'}'>{avg3_val:+.2f}%</div><div class='sub'>上涨率 {up3}/{len(all_3d)} ({up3/len(all_3d)*100:.0f}%)</div></div>")
    if all_1d:
        html.append(f"<div class='stat-card'><div class='label'>1日最大盈利</div><div class='value' style='color:#e74c3c'>{max(all_1d):+.2f}%</div><div class='sub'>最大亏损 {min(all_1d):+.2f}%</div></div>")
        html.append(f"<div class='stat-card'><div class='label'>3日最大盈利</div><div class='value' style='color:#e74c3c'>{max(all_3d):+.2f}%</div><div class='sub'>最大亏损 {min(all_3d):+.2f}%</div></div>")
    html.append("</div>")

    # ── 一、按年汇总 ──
    html.append("<h2>一、按年汇总</h2>")
    html.append("<table><thead><tr>")
    for h in ["年份", "有信号天数", "总信号数", "日均信号", "平均当日涨跌", "平均1日涨跌", "平均3日涨跌", "上涨占比(1日)", "上涨占比(3日)"]:
    ...
    for year_label, dates in year_groups:
        ...
        a1 = safe_avg(year_perfs, "ret_1d")
        a3 = safe_avg(year_perfs, "ret_3d")
        u1 = sum(1 for p in year_perfs if p.get("ret_1d") is not None and p["ret_1d"] > 0)
        u3 = sum(1 for p in year_perfs if p.get("ret_3d") is not None and p["ret_3d"] > 0)
        c1 = sum(1 for p in year_perfs if p.get("ret_1d") is not None)
        c3 = sum(1 for p in year_perfs if p.get("ret_3d") is not None)
        ...
        html.append(f"<td class='{'pos' if (a1 or 0)>=0 else 'neg'}'>{fmt(a1)}</td>")
        html.append(f"<td class='{'pos' if (a3 or 0)>=0 else 'neg'}'>{fmt(a3)}</td>")
        html.append(f"<td>{u1}/{c1} ({u1/c1*100:.0f}%)</td>" if c1 > 0 else "<td>-</td>")
        html.append(f"<td>{u3}/{c3} ({u3/c3*100:.0f}%)</td>" if c3 > 0 else "<td>-</td>")
        html.append("</tr>")
    ...
    a1_all = safe_avg(stock_performances, "ret_1d")
    a3_all = safe_avg(stock_performances, "ret_3d")
    c1_all = sum(1 for p in stock_performances if p.get("ret_1d") is not None)
    c3_all = sum(1 for p in stock_performances if p.get("ret_3d") is not None)
    html.append(f"<td class='{'pos' if (a0_all or 0)>=0 else 'neg'}'><strong>{fmt(a0_all)}</strong></td>")
    html.append(f"<td class='{'pos' if (a1_all or 0)>=0 else 'neg'}'><strong>{fmt(a1_all)}</strong></td>")
    html.append(f"<td class='{'pos' if (a3_all or 0)>=0 else 'neg'}'><strong>{fmt(a3_all)}</strong></td>")
    html.append(f"<td><strong>{up1}/{c1_all} ({up1/c1_all*100:.0f}%)</strong></td>")
    html.append(f"<td><strong>{up3}/{c3_all} ({up3/c3_all*100:.0f}%)</strong></td>")
    ...
    for month_key in month_order:
        ...
        html.append("<table><thead><tr>")
        for h in ["信号日", "选出数", "占比", "平均当日涨跌", "平均1日涨跌", "平均3日涨跌", "上涨占比(1日)", "上涨占比(3日)"]:
            html.append(f"<th>{h}</th>")
        html.append("</tr></thead><tbody>")
        ...
        for d in month_dates:
            ...
            a1 = safe_avg(date_perfs, "ret_1d")
            a3 = safe_avg(date_perfs, "ret_3d")
            u1_d = sum(1 for p in date_perfs if p.get("ret_1d") is not None and p["ret_1d"] > 0)
            u3_d = sum(1 for p in date_perfs if p.get("ret_3d") is not None and p["ret_3d"] > 0)
            c1_d = sum(1 for p in date_perfs if p.get("ret_1d") is not None)
            c3_d = sum(1 for p in date_perfs if p.get("ret_3d") is not None)
            ...
            html.append(f"<td class='{'pos' if (a1 or 0)>=0 else 'neg'}'>{fmt(a1)}</td>")
            html.append(f"<td class='{'pos' if (a3 or 0)>=0 else 'neg'}'>{fmt(a3)}</td>")
            html.append(f"<td>{u1_d}/{c1_d} ({u1_d/c1_d*100:.0f}%)</td>" if c1_d > 0 else "<td>-</td>")
            html.append(f"<td>{u3_d}/{c3_d} ({u3_d/c3_d*100:.0f}%)</td>" if c3_d > 0 else "<td>-</td>")
            html.append("</tr>")
    ...
    html.append("<table><thead><tr><th>评分区间</th><th>数量</th><th>占比</th><th>平均1日收益</th><th>平均3日收益</th></tr></thead><tbody>")
    for lo, hi, label in [(0, 65, "60以下"), (65, 75, "65-75"), (75, 85, "75-85"), (85, 200, "85以上")]:
        bucket = [p for p in stock_performances if lo <= p["score"] < hi]
        if bucket:
            b1 = [p["ret_1d"] for p in bucket if p["ret_1d"] is not None]
            b3 = [p["ret_3d"] for p in bucket if p["ret_3d"] is not None]
            avg1 = sum(b1) / len(b1) if b1 else 0
            avg3 = sum(b3) / len(b3) if b3 else 0
            html.append("<tr>")
            html.append(f"<td>{label}</td><td>{len(bucket)}</td><td>{len(bucket)/total_picked*100:.1f}%</td>")
            html.append(f"<td class='{'pos' if avg1>=0 else 'neg'}'>{avg1:+.2f}%</td>")
            html.append(f"<td class='{'pos' if avg3>=0 else 'neg'}'>{avg3:+.2f}%</td>")
            html.append("</tr>")
    html.append("</tbody></table>")
    ...
    output_path = os.path.join(os.path.dirname(__file__), "oversold-bounce-2022-2025-1d-3d-report.html")
...
def main():
    print("=" * 60)
    print("超跌反弹策略 2022-2025 批量回测（1日/3日持有期）")
    print("=" * 60)
    ...
    for cutoff_date, recs in all_results.items():
        ...
        fp = get_forward_prices(session, codes, cutoff_date, TRACK_DAYS)
    ...
    for cutoff_date in all_results:
        for idx_code in INDICES:
            fp = get_forward_prices(session, [idx_code], cutoff_date, TRACK_DAYS)
