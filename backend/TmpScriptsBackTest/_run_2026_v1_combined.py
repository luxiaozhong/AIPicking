#!/usr/bin/env python3
"""2026 年日频 V1 (T+1 开盘) 回测：M=1/3/5 × Top=3/5 合并图"""
from __future__ import annotations
import json, sys
from pathlib import Path

sys.path.insert(0, str(Path(__file__).resolve().parent.parent))
from TmpScriptsBackTest.backtest_grow_with_money_daily_v1 import run
...
for lb in LOOKBACKS:
    for top in TOPS:
        ...
        data = []; last = 1000.0
        for d in dates:
            if d in nav_map: last = nav_map[d]
...
n = len(fns)
color_map = {}
for (lb, top), clr in COLORS.items():
    color_map[f"M={lb} Top{top}"] = clr

html = f"""<!DOCTYPE html><html lang="zh-CN"><head><meta charset="UTF-8">
<title>grow_with_money V1 · 2026 · 日频 · T+1开盘 · 多参数对比</title>
...
<h1>grow_with_money V1 · 980080 · T+1开盘 · 2026</h1>
<div class="sub">M=1/3/5 × Top=3/5 · {BT_START} ~ {BT_END} · {len(dates)} 个交易日 · 已扣交易费用 · T+1开盘价成交</div>
...
c.setOption({{
    ...
    tooltip: {{
        ...
        formatter: function(p) {{
            ...
            p.forEach(function(x){{s+='<span style="display:inline-block;width:10px;height:10px;border-radius:50%;background:'+x.color+';margin-right:6px"></span>'+x.seriesName+': <b>'+x.value.toFixed(2)+'</b> ('+((x.value-1000)/10).toFixed(1)+'%)<br/>'}});
            return s
        }}
    }},
...
out = Path(__file__).resolve().parent / "backtest_grow_with_money_980080_daily_v1_2026_combined.html"
