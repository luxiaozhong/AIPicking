"""
超跌反弹策略 批量回测脚本（2022-2025）
==============================================
加载全部数据一次，按日切片运行策略，生成 HTML 报表。

用法: cd backend && source venv/bin/activate && python tmpScriptsBackTest/batch_backtest_2022_2025.py
"""
...
def generate_html(all_results, all_index_data, forward_prices, index_forward,
    ...
    html.append("<title>超跌反弹策略 2022-2025 批量回测报告</title>")
    html.append(CSS)
    html.append("</head><body>")

    # 标题
    html.append("<h1>📊 超跌反弹策略 2022-2025 批量回测报告</h1>")
    ...
    for year in ["2022", "2023", "2024", "2025"]:
        t_days = sum(1 for d in oversold_days if d.startswith(year))  # all trading days
    ...
    output_path = os.path.join(os.path.dirname(__file__), "oversold-bounce-2022-2025-report.html")
