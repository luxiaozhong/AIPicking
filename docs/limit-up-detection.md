...
| 板块 | ts_code 前缀 | 涨跌幅限制 | 取整规则 | 说明 |
|------|-------------|------------|----------|------|
| 上海主板 | `60` | ±10% | **四舍五入** | 600xxx, 601xxx, 603xxx, 605xxx |
| 深圳主板 | `00` | ±10% | **四舍五入** | 000xxx, 001xxx, 002xxx, 003xxx |
| 创业板 | `30` | ±20% | **四舍五入** | 300xxx, 301xxx |
| 科创板 | `688`, `689` | ±20% | **四舍五入** | 688xxx, 689xxx |
| 北交所 | `83`, `87`, `43` | ±30% | **涨停取低（截断）** | 83xxxx, 87xxxx, 43xxxx |
| ST 股票 | 任意（名称含 ST） | ±5% | **四舍五入** | *ST, ST 前缀 |

注意：ST 股票判断需要看股票名称（`name` 字段），而非 `ts_code`。ST 股票的涨跌幅限制覆盖板块限制（例如主板 ST 从 ±10% 变为 ±5%）。

### 涨停价计算精度陷阱 ⚠️

**不能简单用 `pre_close × 1.1` 作为涨停价阈值**，原因有两个：

#### 1. 价格最小变动单位导致实际涨幅 ≠ 理论涨幅

A 股价格精确到 **0.01 元（分）**，导致实际涨停价往往**略低于**理论值：

```
例：pre_close = 3.51，主板 ±10%
  理论值：3.51 × 1.10 = 3.861
  四舍五入到分：3.86
  实际涨幅：(3.86 - 3.51) / 3.51 = 9.97%  ← 小于 10%！
```

如果用 `open >= 3.51 * 1.10` 即 `open >= 3.861` 来判断，那 `open = 3.86` 会被错误地判定为"未涨停"，漏判一字板。

#### 2. 不同交易所取整规则不同

| 交易所 | 取整规则 | 示例（pre_close=9.95, +10%） |
|--------|----------|------------------------------|
| 沪深（主板/科创/创业） | **四舍五入** | `round(9.95 × 1.10, 2)` = `round(10.945, 2)` = **10.95** |
| 北交所 | **涨停取低**（直接截断） | `floor(9.95 × 1.30 × 100) / 100` = `floor(12.935, 2)` = **12.93** |

沪深四舍五入可能导致实际涨幅**超过**理论值（9.95 → 10.95，涨幅 10.05% > 10%），但这是交易所允许的。

#### 结论

**涨停价必须按交易所规则精确计算到分，然后用 `open == limit_up_price` 精确匹配，不能用浮点不等号近似比较。**
...
1. **涨停价开盘**：`open == limit_up_price`（开盘价精确等于涨停价）
2. **全天无波动**：`high == low`（最高价 == 最低价，即价格全天未变化）

**原理**：一字板的特征是开盘即涨停、全天封死，K 线图呈现"一字"形态：`open == high == low == close == limit_up_price`。

> ⚠️ 注意：不能用 `open >= pre_close * 1.1` 这种浮点近似比较。涨停价必须用交易所的四舍五入/截断规则精确计算（见上方"涨停价计算精度陷阱"），然后用 `open == limit_up_price` 做等值判断。
...
def _get_limit_up_price(ts_code: str, pre_close: float, name: str = "") -> Optional[float]:
    """根据股票代码和前收盘价计算涨停价（精确到分，符合交易所规则）
    
    沪深交易所（主板/科创/创业）：四舍五入到 0.01 元
    北交所：涨停取低（直接截断第3位小数）
    """
    ...
    if name and "ST" in name.upper():
        limit_pct = 0.05       # ST 股票：±5%
    elif ts_code.startswith(("688", "689")):
        limit_pct = 0.20       # 科创板：±20%
    elif ts_code.startswith(("300", "301")):
        limit_pct = 0.20       # 创业板：±20%
    elif ts_code.startswith(("83", "87", "43")):
        limit_pct = 0.30       # 北交所：±30%
    else:
        limit_pct = 0.10       # 主板（60/00 开头）：±10%
    
    raw = pre_close * (1 + limit_pct)
    
    # 北交所：涨停取低（直接截断到分，不四舍五入）
    if ts_code.startswith(("83", "87", "43")):
...
| 秒板（开盘秒封涨停） | `open < limit` | `high == low`（封死后） | **允许买入** | 开盘未涨停，有买入窗口，之后封死不影响 |
