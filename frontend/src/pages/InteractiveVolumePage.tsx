...
const InteractiveVolumePage: React.FC = () => {
  ...
  useEffect(() => { educationService.getVolumeCases().then(d => { setData(d); if(d.cases.length>0)setCaseId(d.cases[0].id); setLoading(false); }).catch(()=>setLoading(false)); }, []);
  ...
  useEffect(() => { if(!ac)return; setStep(1); setMode('preset'); const e=new Date(ac.date_range.end); const s=new Date(e); s.setMonth(s.getMonth()-pm); load(ac.stock.ts_code,s.toISOString().slice(0,10),ac.date_range.end); }, [caseId]);
  ...
  return <div style={{maxWidth:1100,margin:'0 auto'}}>
    <h2 style={{marginBottom:8}}>成交量 交互学习</h2>
    ...
    {cl?<div style={{textAlign:'center',padding:60}}><Spin size="large"/></div>:<BollingerInteractiveChart data={kl} period={20} multiplier={2} height={mode==='free'?450:400}/>}
    {mode==='preset'&&ac?.steps&&<StepNavigator steps={ac.steps} currentStep={step} onStepChange={setStep}/>}
    <div style={{marginTop:16}}>{cs?.content?<ReactMarkdown>{cs.content}</ReactMarkdown>:mode==='free'?<div style={{color:'#666',fontSize:13}}><p>🔍 <strong>自由探索模式</strong> — 观察成交量柱与 K 线的量价关系。</p></div>:null}</div>
    <div style={{marginTop:16,padding:'12px 16px',background:'#fafafa',borderRadius:8,fontSize:13,color:'#666'}}>💡 <strong>提示</strong>：红色成交量柱 = 当日收涨，绿色 = 当日收跌。柱越高成交量越大。重点关注放量（高柱）的位置与价格走势的配合。</div>
  </div>;
};
export default InteractiveVolumePage;
