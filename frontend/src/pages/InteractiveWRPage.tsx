...
import WRInteractiveChart from '@/components/education/WRInteractiveChart';
...
const InteractiveWRPage: React.FC = () => {
  ...
  const [period, setPeriod] = useState(10); const [step, setStep] = useState(1); const [pm, setPm] = useState(6);

  useEffect(() => { educationService.getWRCases().then(d => { setData(d); if(d.cases.length>0)setCaseId(d.cases[0].id); setLoading(false); }).catch(()=>setLoading(false)); }, []);
  const ac = data?.cases?.find((c:any)=>c.id===caseId); const dp: any = data?.default_params || { period: 10 };
  const load = useCallback(async (ts:string,st:string,en:string) => { setCl(true); try { const s=st.replace(/-/g,''),e=en.replace(/-/g,''); const r=await stockService.getKLine(ts,Math.min(365,Math.ceil((new Date(en).getTime()-new Date(st).getTime())/86400000)+30)); const i=r.items||[]; const f=i.filter((x:KLineItem)=>x.trade_date>=s&&x.trade_date<=e);setKl(f.length>0?f:i); } finally { setCl(false); } }, []);
  useEffect(() => { if(!ac)return; setPeriod(dp.period); setStep(1); setMode('preset'); const e=new Date(ac.date_range.end); const s=new Date(e); s.setMonth(s.getMonth()-pm); load(ac.stock.ts_code,s.toISOString().slice(0,10),ac.date_range.end); }, [caseId]);
  ...
  return <div style={{maxWidth:1100,margin:'0 auto'}}>
    <h2 style={{marginBottom:8}}>WR 威廉指标 交互学习</h2>
    ...
    {cl?<div style={{textAlign:'center',padding:60}}><Spin size="large"/></div>:<WRInteractiveChart data={kl} period={period} height={mode==='free'?450:400}/>}
    {mode==='preset'&&ac?.steps&&<StepNavigator steps={ac.steps} currentStep={step} onStepChange={setStep}/>}
    <div style={{marginTop:16}}>{cs?.content?<ReactMarkdown>{cs.content}</ReactMarkdown>:mode==='free'?<div style={{color:'#666',fontSize:13}}><p>🔍 <strong>自由探索模式</strong> — WR 线已标注超买(20)/超卖(80)参考线。🔔 WR 方向与 RSI 相反：值越大越超卖！</p></div>:null}</div>
    <div style={{marginTop:16,padding:'12px 16px',background:'#fafafa',borderRadius:8,display:'flex',alignItems:'center',gap:24,flexWrap:'wrap'}}>
      ...
      <div style={{display:'flex',alignItems:'center',gap:8}}><span style={{fontSize:12,color:'#666'}}>周期</span><Slider style={{width:100,margin:0}} min={2} max={30} value={period} onChange={setPeriod}/><strong style={{fontSize:12,minWidth:20}}>{period}</strong></div>
      <Button size="small" icon={<UndoOutlined />} onClick={()=>setPeriod(dp.period)}>恢复默认</Button>
    </div>
  </div>;
};
export default InteractiveWRPage;
